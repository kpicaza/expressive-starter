 ...
 */
final class ExpressiveAuraConfig implements ContainerConfigInterface
{
    /**
     * @var array
     */
    ...
    private function marshalDelegators(Container $container, array $dependencies)
    {
        foreach ($dependencies['delegators'] as $service => $delegatorNames) {
            ...
            $container->set($delegatorFactory, function () use ($delegatorNames, $factory) {
                return new ExpressiveAuraDelegatorFactory($delegatorNames, $factory);
            });
            $container->set(
                $service,
